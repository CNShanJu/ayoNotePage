...
`向 Set 加入值的时候，不会发生类型转换`，所以`5`和`"5"`是两个不同的值。Set 内部判断两个值是否不同，使用的算法叫做“Same-value-zero equality”，它`类似于精确相等运算符`（`===`），主要的`区别`是向` Set` 加入值时认为`NaN等于自身`，而`精确相等运算符`认为`NaN不等于自身`。

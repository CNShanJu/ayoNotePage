...
因为Java是**面向对象**的语言，一个程序的基本单位就是`class`，`class`是关键字，这里定义的`class`名字就是`Hello`：
...
这里的方法名是`main`，返回值是`void`表示**没有任何返回值**。

我们注意到`public`除了可以修饰`class`外，也可以修饰方法。

而关键字`static`是另一个修饰符，它表示**静态方法**，后面我们会讲解方法的类型，

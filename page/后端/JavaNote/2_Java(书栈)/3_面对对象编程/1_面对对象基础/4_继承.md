...
![继承 - 图1](img/4_继承/dd8e3d729d45288e341c956af580c15f.png)

运行上面的代码，会得到一个编译错误，大意是在`Student`的构造方法中，无法调用`Person`的构造方法。

这是因为在Java中，任何`class`的构造方法，`第一行语句`必须是调用`父类的构造方法`。如果没有明确地调用父类的构造方法，编译器会帮我们自动加一句`super();`，所以，`Student`类的构造方法实际上是这样：
...
`向上转型`实际上是把一个`子类型`安全地变为更加抽象的`父类型`：
...
和向上转型相反，如果把一个`父类类型`**强制转型**为`子类类型`，就是`向下转型`（downcasting）。例如：
...
![继承 - 图2](img/4_继承/fc21590dcca81db8581c5f0e9f372ea7.png)

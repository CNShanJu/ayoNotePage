...
- `Field getField(name)`：根据字段名获取某个public的field（`包括`父类）
- `Field getDeclaredField(name)`：根据字段名获取当前类的某个field（`不包括`父类）
- `Field[] getFields()`：获取所有public的field（`包括`父类）
- `Field[] getDeclaredFields()`：获取当前类的所有field（`不包括`父类）

我们来看一下示例代码：

![访问字段 - 图1](img/2_访问字段/e19b94a4725894992930c181c6a06e70.png)
...
![访问字段 - 图2](img/2_访问字段/834975a37a60fde7dee62d0ef4de7235.png)
...
![访问字段 - 图3](img/2_访问字段/23581bb8bdc62d4e3b57760f2850cfd8.png)
